...
abstract class MultiprocessOld extends \System\Service_Controller_Shell
{
    public const PROC_STATUS_UNKNOWN = -1;
    public const PROC_STATUS_TRUE = 2;
    public const PROC_STATUS_FALSE = 3;
    public const PROC_STATUS_EXCEPTION = 4;
    ...
    final protected function _RunOnce()
    {
        ...
        $writed = fwrite($this->_subprocess_pipe, serialize($data));
        $this->log_processes('Writed: ' . $writed . '. May be count of processes has been decreased.');
        fclose($this->_subprocess_pipe);
    ...
    private function _waitOne()
    {
        ...
        return [$_free_proc_num, (int) (isset($status) && $status == self::PROC_STATUS_TRUE)];
    }
