...
/* @(#) $Header: /sources/phpprintipp/phpprintipp/php_classes/http_class.php,v 1.9 2013/04/15 20:47:15 harding Exp $ */
 ...
 * $Id: http_class.php,v 1.9 2013/04/15 20:47:15 harding Exp $
 */
/**
 ...
 public $port;
   ...
   default:
    ...
    break;
        ...
        self::Open($arguments);
        $error =
   sprintf(_('Streaming request failed to %s after a try to authenticate'),
     $url);
        $result = self::_StreamRequest($arguments);
    ...
    private function _StreamRequest($arguments)
    {
      ...
      $this->_HttpError(sprintf(_('%s: file is not readable'), $value),
        E_USER_WARNING);
                ...
                return
     $this->_HttpError(sprintf(_('%s: not a valid argument for content'), $type),
       E_USER_WARNING);
            }
   ...
   array_merge($this->headers, $this->arguments['Headers']);
        ...
        if ($this->arguments['RequestMethod'] != 'POST') {
            return
    $this->_HttpError(sprintf(_('%s: method not implemented'),
