...
<form method="get" class="form-horizontal" role="form" style="text-align: left;">
            ...
            <select class="form-control" name="groupId" id="i_groupId">
                ...
                <?php foreach ($vars['groups']['items'] as $item): ?>
                    <option data-image="<?= $item['photo_50']; ?>"
                            value="<?= $item['id']; ?>"<?php if (isset($group) && $item['id'] == $group->ownerId): ?> selected="selected"<?php endif; ?>>
                        <?= $item['name']; ?>
                    </option>
                <?php endforeach; ?>
            </select>
    ...
    <div class="text-right">
        <button onclick="grabber.saveClick();" type="button" class="button-green btn-block">Активировать группу</button>
    </div>
</form>
<script>
