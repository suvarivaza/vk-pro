...
?>
<ul class="breadcrumb">
    <li><a href="/tasks/special/<?= $special->specialId; ?>/all/1"><?= $special->title; ?></a></li>
    <li><strong>Заказ подписчиков в группу</strong></li>
</ul>

<form id="i_form_task_add" action="" method="POST" enctype="multipart/form-data" target="_self" class="form-horizontal"
      role="form">
    <input id="i_form_task_add_action" type="hidden" name="action" value="save"/>
    <input type="hidden" id="i_form_task_add_taskId" name="taskId" value=""/>
    <input type="radio" name="type" value="join" id="join" autocomplete="off" checked style="display: none;">
    <?php if ($vars['errors']): ?>
    ...
    <div id="i_div_task_url" class="form-group">
        ...
        <div class="col-sm-4">
            ...
            <input type="text" class="form-control" id="i_task_url" name="url" placeholder="" value="<?= $task->url; ?>"
                   disabled="disabled"/>
        </div>
        <div class="col-sm-1">
...
<script>
    special.prices = <?= json_encode($vars['prices']); ?>;
    special.percents = <?= json_encode($vars['percents']); ?>;
    special.percentsVals = <?= json_encode($vars['percentsVals']); ?>;
    special.userCityId = <?= intval($user->cityId); ?>;
