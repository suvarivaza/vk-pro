<h3>
    Подсказки при добавлении задания
...
<script>
    ...
    $('.c-main-menu-ul-li').click(function () {
        location.href = '/admin/tasks/tips/' + $(this).data('type');
    });
</script>
...
<form method="post">
    ...
    continue;
            ...
            <div class="form-group c-form-tinymce" id="i_tip_<?= $vars['type']; ?>_<?= $vkType; ?>">
                <div class="col-sm-12">
                    <textarea name="tip_<?= $vars['type']; ?>_<?= $vkType; ?>"
                              class="tinymce form-control"><?= $vars['texts']['tip_' . $vars['type'] . '_' . $vkType]; ?></textarea>
                </div>
            </div>
        <?php endforeach; ?>
    <?php else: ?>
        <div class="form-group">
            <div class="col-sm-12">
                <textarea id="i_tip_<?= $vars['type']; ?>_<?= $vkType; ?>" name="tip_<?= $vars['type']; ?>"
