<?php
/** @var \Service\Posting\Model_Postings_Posting $posting */
$posting = $vars['posting'];
$slots = $posting->getSlots();
/** @var \Service\Posting\Model_Groups_Group[] $groups */
$groups = $vars['groups'];
$list = [];
...
?>
    <div class="pull-right">
        <a href="/faq/list/8" target="_blank" style="padding: 7px; display: block;"
           title="Помощь по сервису Автопопстинг">
...
<?php foreach ($list as $months => $count): ?>
    <div class="alert alert-info">
        <div class="pull-right">
            <button class="button-green c_posting_slot_active" data-months="<?= $months; ?>"
                    id="i_posting_slot_active_<?= $months; ?>">Активировать слот
            </button>
        </div>
...
<?php foreach ($groups as $group): ?>
    <div class="c-task-detail c-posting-group" data-posting-group-id="<?= $group->groupId; ?>">
...
<?php endforeach; ?>
    ...
    <div id="i_posting_group_add" class="row c-task-detail c-posting-group-add">
        ...
        <div class="col-sm-10">
            <h3>Добавить группу для автопостинга</h3>
        </div>
    </div>
