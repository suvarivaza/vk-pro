...
class Controller_State_Client_Social_Auth extends Controller_State_Client
{
    ...
    public function actionPost()
    {
        ...
        if ($token) {
            ...
            //здесь ставятся пользователи с айди от 1 -3 Почему именно эти???
            $this->VK->init($this->factoryUsers->users->getById(random_int(1, 3)));

            $result = file_get_contents('http://ulogin.ru/token.php?token=' . $token . '&host=' . $host);
            ...
            $user = $this->factoryUsers->users->getByUid($json['uid'], true);
            ...
            if ($user === null) {
                ...
                $result = $this->VK->getUser($json['uid'], $this->check_access_token, $fields);
                $data = $result;
                ...
                $result = $this->VK->getPosts($json['uid'], 1,$this->check_access_token);
                ...
                $user->relation = isset($data[0]['relation']) ? (int) $data[0]['relation'] : 0;

                $friends = $this->VK->getFriends($json['uid'], $this->check_access_token);

                $followersCount = $friends['count'] + ($data[0]['followers_count'] ?? 0);
                ...
                if (isset($data[0]['personal']) && is_array($data[0]['personal']) && count($data[0]['personal'])) {
                    ++$user->partCount;
                }

                if (isset($data[0]['military']) && is_array($data[0]['military']) && count($data[0]['military'])) {
                    ++$user->partCount;
                }

                if (isset($data[0]['career']) && is_array($data[0]['career']) && count($data[0]['career'])) {
                    ++$user->partCount;
                }

                if (isset($data[0]['relatives']) && is_array($data[0]['relatives']) && count($data[0]['relatives'])) {
                    ++$user->partCount;
                }
                ...
                $this->_application->Log->Log(\Service\Logs\Model_Config::USER_LOGIN, $user->userId, $user->userId, ['login' => $user->login, 'email' => $user->email]);
                $this->_application->setUserToCookie($user);
