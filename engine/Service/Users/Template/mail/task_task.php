<?php
/** @var \Service\Users\Model_Users_User $user */
$user = $vars['user'];
$photo = $user->getPhotos();
/** @var \Service\Tasks\Model_Tasks_Task[] $tasks */
$tasks = $vars['tasks'];
?>
<div style="background-color: #5b7196; height: 74px; box-shadow: 3px 3px 9px #999">
    <table style="width: 100%; height: 100%; border: 0;" border="0" cellpadding="10">
        <tr>
            <td>
                <a href="https://vk-pro.top">
                    <img style="max-height: 40px;"
                         src="data:image/png;base64,"/>
                </a>
            </td>
            <td style="text-align: right; color: #ffffff;">
                <table align="right" cellpadding="0" cellspacing="5">
                    <tr>
                        <td>
                            <img style="border-radius: 25px;"
                                 src="data:image/jpg;base64,<?= base64_encode(file_get_contents(IMAGES_PATH . $photo['small']['path'] . $photo['small']['file'])); ?>"/>
                        </td>
                        <td>
                            <?= $user->name; ?>
                        </td>
                    </tr>
                </table>
            </td>
        </tr>
    </table>
</div>
<br/>
<h1>Добрый день, <?= $user->name; ?>!</h1>

<div>С радостью сообщаем, что мы разработали и ввели в работу автоматическое выполнение заданий на нашем сервисе.</div>
<div>Бот может выполнять задания за Вас автоматически, и зарабатывать Вам баллы на сервисе 24/7 без вашего участия.
</div>
<div>В связи с появлением бота, <strong>скорость выполнения заданий значительно повысилась</strong>, без потери
    качества.
</div>
<div>Уведомляем Вас, что бот строго следует всем лимитам при выполнении заданий, и работает очень аккуратно, дабы не
    допустить блокировку Вашей страницы Вк за “подозрительную активность”.
</div>

<div>С радостью сообщаем, что мы разработали и ввели в работу нового бота на нашем сервисе.</div>
<div>Бот может выполнять задания за Вас автоматически, и зарабатывать Вам баллы на сервисе 24/7.</div>
<div>Для получения баллов на нашем сервисе, Вам не нужно монотонно выполнять задания одно за другим.</div>
<div>Достаточно лишь войти на сервис, включить бота и он начнет зарабатывать Вам баллы. А Вы сможете потратить их на
    любые свои задания.
</div>
<br/>
<div style="text-align: center;">
    <a style="display: inline-block; padding: 20px; background-color: #5b7196; border-radius: 40px; color: #ffffff; text-decoration: none;"
       href="https://vk-pro.top">Перейти на сайт VK-PRO.TOP</a>
</div>
<br/>
<div>На текущий момент бот находится в тестовом режиме и весь его функционал абсолютно бесплатен.</div>
<div>Приглашаем Вас войти на сервис под своим аккаунтом, и активировать бота в разделе “Все задания”, или в своём личном
    кабинете.
</div>
<div>Уведомляем Вас, что бот строго следует всем лимитам при выполнении заданий, и работает очень аккуратно, дабы не
    допустить блокировку Вашей страницы Вк за “подозрительную активность”.
</div>
<br/>
<?php if (count($tasks) > 0): ?>
    <div>На данный момент были выполнены следующие Ваши задания:</div>
    <table style="width: 100%;" cellpadding="5">
        <tr>
            <th></th>
            <th style="text-align: left;">Тип задания</th>
            <th style="text-align: left;">Описание задания</th>
            <th style="text-align: right;">Выполнено</th>
        </tr>
        <?php foreach ($tasks as $task): $photo = $task->getPhoto(); ?>
            <tr style="border-bottom: 1px solid #808080;">
                <td style="border-bottom: 1px solid #5b7196">
                    <?php if (isset($photo['small'])): ?>
                        <img src="https://vk-pro.top<?= $photo['small']['url']; ?>"
                             style="width: 48px;height: 48px; object-fit: cover;"/>
                    <?php else: ?>
                        <img src="/img/icons/icon-android.png" style="width: 32px;"/>
                    <?php endif; ?>
                </td>
                <td style="text-align: left;border-bottom: 1px solid #5b7196">
                    <?= isset($vars['titles'][$task->type]['vkTypes']) ? $vars['titles'][$task->type]['vkTypes'][$task->vkType] : $vars['titles'][$task->type]['title']; ?>
                </td>
                <td style="text-align: left;border-bottom: 1px solid #5b7196">
                    <?= $task->title; ?>
                </td>
                <td style="text-align: right;border-bottom: 1px solid #5b7196">
                    выполнено <strong><?= $task->countReady; ?></strong> из <strong><?= $task->count; ?></strong>
                </td>
            </tr>
        <?php endforeach; ?>
    </table>
<?php endif; ?>
<br/>
<strong>Желаем приятной работы на нашем сервисе.</strong>
