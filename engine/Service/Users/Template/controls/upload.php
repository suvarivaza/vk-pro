...
<div class="uploader_container" id="<?= $params['container']; ?>">
    ...
    foreach ($params['list'] as $file): $count++;
        $sorting[] = $file['key']; ?>
        <div class="uploader_file" id="<?= $file['key']; ?>"><a href="<?= $file['url']; ?>" target="_blank"><img
                        src="<?= $file['url_preview']; ?>"/></a>
            <div class="uploader_close" title="Удалить"
                 onclick="uploader.fileDelete('<?= $file['key']; ?>', '<?= $file['key']; ?>')"></div>
        </div>
    <?php endforeach; ?>
